...
class Game:
    ...
    def show_custom(self):
        ...
        dialog = CustomConfigDialog(self.window, self)
    ...
    def custom_board_analysis(self): #needs changing to accomodate the new board validity check function
        ...
        dialog = CustomConfigDialog(self.window, self) #facilitates the class CustomConfigDialog
        self.window.wait_window(dialog)
        if dialog.result is None:
            return
...
class CustomConfigDialog(tk.Toplevel):
    ...
    Warns if board size >30 rows and 70 columns, prevents above 9800.
    """
